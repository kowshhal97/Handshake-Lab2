...
router.get('/job/:job_id', (request, response) => {
...
router.get('/getJob/:studentId/:jobId', (request, response) => {
    const student_id = request.params.studentId;
    const job_id=request.params.jobId
    console.log(student_id,job_id)
    try {
        dbConnectionPool.query(`SELECT * from application_information_table WHERE (student_id=${student_id} and job_id=${job_id})`, (error, result) => {
            if (error) {
                console.log(error);
                return response.status(500).send('Server Error');
            }
...
router.post('/', (request, response) => {
    ...
    try {
        dbConnectionPool.query(`INSERT into application_information_table (application_status, application_date, student_id,company_id, job_id) VALUES ('${application_status}', '${application_date}', '${student_id}', '${company_id}', '${job_id}')`,
            (error, result) => {
            if (error) {
                console.log(error);
                return response.status(500).send('Server Error');
